...
def balance(ship_grid, containers):
    ...
    while (balanced is False):
        ...
        if left_balance > right_balance:
            curr_containers = [loc for loc in containers if
                               loc[1] < halfway_line and ship_grid[loc[0]][loc[1]].container is not None]
        else:
            curr_containers = [loc for loc in containers if
                               loc[1] >= halfway_line and ship_grid[loc[0]][loc[1]].container is not None]
        ...
        for container_loc in curr_containers:
            ...
            balance_update.append(
                (container_loc, close_to_balance(ship_grid, container_loc, left_balance, right_balance)))
...
def sift(ship_grid, containers, store_goals):
    ...
    container_weights = sorted(
        [(container, ship_grid[container[0]][container[1]].container) for container in containers],
        key=lambda container: container[1].weight, reverse=True)
